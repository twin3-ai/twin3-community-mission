...
「30 題」定義為一個玩家的 **Mission Cycle**，不是永久上限。

- 每位玩家進入一個由 30 題組成的任務週期。
- 題目依分類與難度平衡分配，不是完全隨機。
- 每場從該週期抽出 10 題。
- 優先出現尚未回答的題目。
- 答錯題目可在後續場次重新出現，但連續兩場不重複。
- 完成 30 題後，系統建立下一個 Mission Cycle。
- 下一週期優先使用未見過、已改版或需要複習的題目。
- 同一場不得出現重複題目。
...
- 後台設定每題首次掌握的 `$PoC`。
- 玩家第一次對某個題目版本答對時，才可獲得該題獎勵。
- 重複答對已掌握題目可增加學習紀錄，但不重複發放。
- 單人與多人使用相同單價。
- 排名與速度不增加 `$PoC`。
...
4. 題目版本有效。
5. 答案在時間內送達並由伺服器判定正確。
6. 該玩家尚未取得該題目版本的首次掌握獎勵。
7. 結算事件尚未處理。

唯一結算鍵：

`mission_id + discord_uid + question_version_id`
...
1. 一人也能準時完成任務，不顯示假玩家。
2. 每場正確抽出 10 題且不重複。
3. 每人 30 題 Mission Cycle 可完成並正確輪替。
4. 正確答案不出現在前端程式。
